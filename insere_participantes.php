<?php
	...
	$login = htmlspecialchars($_POST["loginpart"]);
	...
	if ((in_array($extensao, $permissoes))
		...
		try
		{
			...
			$part->setSenha($senha);
			$inseriu = $part->insereParticipante();
			if ($inseriu)
			{
				echo "<h2>Participante cadastrado com sucesso.<h2>";
		...
		{
			echo "<h2>Erro ao incluir participante. ".$e->getMessage()."</h2>";
		}
		echo "</body>";	
		include_once("rodape.html");
?>